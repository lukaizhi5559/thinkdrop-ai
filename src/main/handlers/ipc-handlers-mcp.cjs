 ...
 * 
 * Thin routing layer that forwards requests to MCP orchestrator.
 * Provides automatic fallback to local agents if MCP unavailable.
 */

const { ipcMain } = require('electron');
const { getMCPOrchestrator, initializeMCP } = require('../services/mcp');
...
function registerMCPHandlers() {
    ...
    try {
      ...
      const result = await orchestrator.classifyIntent(
        ...
        async () => {
          ...
          return await agents.intentParser.parse(message);
        }
      );
   ...
   */
  ipcMain.handle('mcp:memory:store', async (event, { data, context = {} }) => {
    try {
      const orchestrator = getMCPOrchestrator();
      ...
      const result = await orchestrator.executeMemoryOperation(
        ...
        async () => {
          ...
          const agents = getFallbackAgents();
          return await agents.userMemory.storeMemory(data);
        }
      );
  ...
  /**
   * Search memory via MCP (UserMemory service)
   */
  ipcMain.handle('mcp:memory:search', async (event, { query, options = {}, context = {} }) => {
    try {
      const orchestrator = getMCPOrchestrator();
      ...
      const result = await orchestrator.executeMemoryOperation(
        ...
        async () => {
          // Fallback to local agent
          const agents = getFallbackAgents();
          return await agents.userMemory.searchMemories(query, options);
        }
      );
  ...
  /**
   * Retrieve memory via MCP (UserMemory service)
   */
  ipcMain.handle('mcp:memory:retrieve', async (event, { memoryId, context = {} }) => {
    try {
      const orchestrator = getMCPOrchestrator();
      ...
      const result = await orchestrator.executeMemoryOperation(
        ...
        async () => {
          // Fallback to local agent
          const agents = getFallbackAgents();
          return await agents.userMemory.getMemoryById(memoryId);
        }
      );
