...
export const ConversationSidebar: React.FC<ConversationSidebarProps> = ({ className = '' }) => {
  ...
  // Use signal value for reactivity
  ...
  // Listen for message updates to refresh session list
  React.useEffect(() => {
    const handleMessageAdded = async (_: any, data: { sessionId: string }) => {
      console.log('📨 [ConversationSidebar] Message added to session:', data.sessionId);
      // Reload sessions to update message counts and last message previews
      try {
        await loadSessions();
        console.log('✅ [ConversationSidebar] Sessions reloaded after message added');
      } catch (error) {
        console.error('❌ [ConversationSidebar] Failed to reload sessions:', error);
      }
    };

    if ((window as any).electron?.ipcRenderer) {
      (window as any).electron.ipcRenderer.on('conversation:message-added', handleMessageAdded);
    }

    return () => {
      if ((window as any).electron?.ipcRenderer?.removeListener) {
        (window as any).electron.ipcRenderer.removeListener('conversation:message-added', handleMessageAdded);
      }
    };
  }, []);
  ...
  const handleNewChat = useCallback(async () => {
    console.log('🆕 [ConversationSidebar] handleNewChat called');
    try {
      console.log('🆕 [ConversationSidebar] Creating new session...');
      const sessionId = await createSession('user-initiated', {
        title: `Chat ${sessions.length + 1}`
      });
      
      console.log('🆕 [ConversationSidebar] Session created:', sessionId);
  ...
  return (
    <>
      {/* Backdrop - always visible when sidebar is open */}
      {isSidebarOpen && (
        <div 
    ...
    >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700/50">
          ...
          <div className="flex items-center gap-2 relative z-10">
            <button
              onClick={(e) => {
                console.log('🖱️ [PLUS BUTTON] onClick fired!', e);
                handleNewChat();
              }}
              onMouseDown={(e) => {
                console.log('🖱️ [PLUS BUTTON] onMouseDown fired!', e);
              }}
              onMouseEnter={() => {
                console.log('🖱️ [PLUS BUTTON] onMouseEnter fired!');
              }}
              disabled={isLoading}
              className="p-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white transition-colors disabled:opacity-50 relative z-10"
              title="New Chat"
              style={{ pointerEvents: 'auto' }}
            >
            ...
            <button
              ...
              className="p-2 rounded-lg hover:bg-gray-700/50 text-gray-400 hover:text-white transition-colors md:hidden relative z-10"
              title="Close Sidebar"
              style={{ pointerEvents: 'auto' }}
            >
            ...
            <button
              onClick={(e) => {
                console.log('🖱️ [X BUTTON] onClick fired!', e);
                e.preventDefault();
                e.stopPropagation();
                closeSidebar();
              }}
              onMouseDown={(e) => {
                console.log('🖱️ [X BUTTON] onMouseDown fired!', e);
              }}
              onMouseEnter={() => {
                console.log('🖱️ [X BUTTON] onMouseEnter fired!');
              }}
              className="p-2 rounded-lg hover:bg-gray-700/50 text-gray-400 hover:text-white transition-colors hidden md:block relative z-10"
              title="Close Sidebar"
              style={{ pointerEvents: 'auto' }}
            >
              <X className="h-4 w-4" />
