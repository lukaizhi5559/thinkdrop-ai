...
import type { ConversationSession, ChatMessage } from '../types/conversation';

// Session Management Actions
export const switchToSession = async (sessionId: string): Promise<void> => {
...
export const loadMessages = async (sessionId: string, options?: {
  ...
  try {
    ...
    if ((window.electronAPI as any)?.mcpCall) {
      ...
      console.log('🔍 [SIGNALS] loadMessages response:', {
      ...
      if (result.success && result.data?.success && messagesData) {
        console.log('✅ [SIGNALS] Loaded messages:', messagesData.length, 'for session:', sessionId);
        console.log('📝 [SIGNALS] Message IDs:', messagesData.map((m: any) => m.id));
        messages.value = {
          ...messages.value,
          [sessionId]: messagesData
        };
        console.log('📊 [SIGNALS] Updated messages signal, total sessions with messages:', Object.keys(messages.value).length);
        return messagesData;
      } else {
...
export const addMessage = async (sessionId: string, message: Omit<ChatMessage, 'id' | 'timestamp'>): Promise<ChatMessage | null> => {
  ...
  try {
    if ((window.electronAPI as any)?.mcpCall) {
      ...
      // If we got here, the message wasn't added successfully
      console.warn('⚠️ [SIGNALS] Message add failed - no messageId in response');
      return null;
    }
    
    // mcpCall not available
    console.error('❌ [SIGNALS] mcpCall not available');
    return null;
  } catch (err) {
...
// Session management actions
export const updateSession = async (sessionId: string, updates: { title?: string; [key: string]: any }): Promise<boolean> => {
...
export const hibernateSession = async (sessionId: string): Promise<boolean> => {
